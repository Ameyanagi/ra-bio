...
`ra-bio` is published to PyPI as a public runtime library and bundled
microorganism risk-reference dataset. Service wrappers, scrapers, and
deployment-specific projects stay GitHub-only.
...
- Keep only source files and intended runtime data under `src/ra_bio/data/`.
...
2. Refresh `src/ra_bio/data/bio.sqlite3` when releasing a data update.
